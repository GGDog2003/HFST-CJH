...
class CohfT(nn.Module):
    def __init__(self, in_channel, img_size, hidden_dim, layer_num, scale=2, window_size=5, kernel_size=3,
        ...
        self.ddsf = DualDomainSparseFusion(in_channel=in_channel, hidden_dim=hidden_dim)
   ...
   """
        ...
        for SLCC in self.SLCC_block:
            Fs2 = SLCC([Fs2, enhanced_ref])
        FL0 = res_Fs2 + Fs2[:, :C, :, :]
        # 在 HFST 原始结构融合后追加双域稀疏融合，补强频率域建模能力。
        FL0 = self.ddsf(FL0, fused_high, confidence, diff_map)
